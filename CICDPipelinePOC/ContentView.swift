...
enum BuildMetadata {
    ...
    static let commit = infoValue(
        key: "CI_COMMIT_SHA",
        fallback: "Local build"
    )

    static let branch = infoValue(
        key: "CI_BRANCH",
        fallback: "Local"
    )

    static let workflow = infoValue(
        key: "CI_WORKFLOW",
        fallback: "Xcode"
    )

    static let runNumber = infoValue(
        key: "CI_RUN_NUMBER",
        fallback: "Local"
    )

    static let buildDate = infoValue(
        key: "CI_BUILD_DATE",
    ...
    private var triggerInformation: some View {
        GroupBox {
            VStack(alignment: .leading, spacing: 12) {
                Label(
                    "Pull request: build and unit tests",
                    systemImage: "checkmark.circle"
                )

                Label(
                    "Push to main: test, sign, build, and upload",
                    systemImage: "paperplane.circle"
                )

                Label(
                    "Secrets remain in GitHub Actions",
